...
class TestGetBugs:
    ...
    @responses.activate
    def test_get_bugs_timeout_parameter(self, bug_searcher):
        """Test get_bugs uses timeout parameter."""
        responses.add(
            responses.GET,
            "https://bugzilla.mozilla.org/rest/bug",
            json={"bugs": []},
        ...
        with patch("treeherder.perf.auto_perf_sheriffing.bug_searcher.requests.get") as mock_get:
            mock_response = Mock()
    ...
    @responses.activate
    def test_get_bugs_verifies_ssl(self, bug_searcher):
        """Test get_bugs uses SSL verification."""
        responses.add(
            responses.GET,
            "https://bugzilla.mozilla.org/rest/bug",
            json={"bugs": []},
    ...
    @responses.activate
    def test_get_bugs_with_history_field(self, bug_searcher):
        """Test get_bugs can request bug history."""
        expected_response = {
            "bugs": [
                {
                    "id": 123456,
        ...
        assert result == expected_response
        assert "history" in result["bugs"][0]
