...
class TestBugManager:
    ...
    @responses.activate
    def test_create_bug_timeout_parameter(self, bug_manager):
        """Test _create uses timeout parameter."""
        responses.add(
            responses.POST,
            "https://bugzilla.mozilla.org/rest/bug",
            json={"id": 123},
        ...
        # Mock requests.post to verify timeout
        with patch(
            "treeherder.perf.auto_perf_sheriffing.base_bug_manager.requests.post"
        ) as mock_post:
            mock_response = Mock()
    ...
    @responses.activate
    def test_modify_bug_timeout_parameter(self, bug_manager):
        """Test _modify uses timeout parameter."""
        responses.add(
            responses.PUT,
            "https://bugzilla.mozilla.org/rest/bug/123",
            json={"bugs": []},
            status=200,
        )
        ...
        # Mock requests.put to verify timeout
        with patch(
            "treeherder.perf.auto_perf_sheriffing.base_bug_manager.requests.put"
        ) as mock_put:
            mock_response = Mock()
            ...
            bug_manager._modify(123, changes)
    ...
    @responses.activate
    def test_create_bug_verifies_ssl(self, bug_manager):
        """Test _create uses SSL verification."""
        responses.add(
            responses.POST,
            "https://bugzilla.mozilla.org/rest/bug",
            json={"id": 123},
    ...
    @responses.activate
    def test_modify_bug_verifies_ssl(self, bug_manager):
        """Test _modify uses SSL verification."""
        responses.add(
            responses.PUT,
            "https://bugzilla.mozilla.org/rest/bug/123",
            json={"bugs": []},
