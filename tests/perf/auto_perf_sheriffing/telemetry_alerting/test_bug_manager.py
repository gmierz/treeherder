...
class TestTelemetryBugManager:
    ...
    def test_file_bug_sets_correct_fields(self, bug_manager, mock_probe, telemetry_alert_obj):
        ...
        assert request_body["flags"][0]["requestee"] == "gmierzwinski@mozilla.com"
...
class TestTelemetryBugContent:
    ...
    def test_build_bug_content_uses_channel_to_repo_mapping(self, bug_content, telemetry_alert_obj):
        """Test that build_bug_content uses channel to repo mapping."""
        telemetry_alert_obj.telemetry_signature.channel = "Nightly"

        with patch(
            "treeherder.perf.auto_perf_sheriffing.telemetry_alerting.bug_manager.CHANNEL_TO_REPO_MAPPING",
            {"Nightly": "mozilla-central", "Beta": "mozilla-beta"},
        ):
            result = bug_content.build_bug_content(telemetry_alert_obj)

            # Should successfully build content
            assert result["title"] is not None
            assert result["description"] is not None

    def test_build_bug_content_defaults_to_mozilla_central_for_unknown_channel(
        self, bug_content, telemetry_alert_obj
    ):
        """Test that build_bug_content defaults to mozilla-central for unknown channels."""
        telemetry_alert_obj.telemetry_signature.channel = "UnknownChannel"
        ...
        with patch(
            ...
            # Should still successfully build content with default
            assert result["title"] is not None
            assert result["description"] is not None
